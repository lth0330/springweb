...
class SubClass extends SuperClass{  // 하위 클래스
    ...
    @Override void method() {}         // 상위 클래스로부터 물려받은 메소드 정의
    @Deprecated void method2(){}        // 더 이상 사용하지 않은 메소드를 알리는 어노테이션
}
...
@interface MyAnnotatin{
    String value1() default "";    // 구현부가 없는 추상 메소드
}

// [어노테이션 사용하기] : 내가 만든 어노테이션 사용
class TestClass{
    @MyAnnotatin(value1 = "주입하는 값")     // @어노테이션이란? 해당 기술을 사용하기 위한 설명
...
public class Exam1 {
    public static void main(String[] args) {
        ...
        /*
            ...
            3. 종류
                3-1) 표준 어노테이션 : 자바에서 기본적으로 제공하는 이노테이션
                        @Override : 상위 클래스로부터 메소드를 재정의
                3-2) 메타 어노테이션 : 코드를 정의하거나 동작을 제어할 때 사용되는 어노테이션
                        스프링 프레임워크
            4. 어노테이션 만들기
                1. @interface 어노테이션{}
                2. 인터페이스 위에 유지기간, 실행 위치 설정
                    1] @Retention(RetentionPolicy.RUNTIME) : 프로그램 실행중 유지.
                       @Retention(RetentionPolicy.CLASS) : .class 파일에 포함.
                       @Retention(RetentionPolicy.SOURCE) : 컴파일(코드 -> 번역) 이후에 까지만 유지
                    2] @Target(ElementType.TYPE) : 클래스에서 사용
                       @Target(ElementType.METHOD) : 메소드에서 사용
                       @Target(ElementType.FIELD) : 맴버변수에서 사용
                       등등
            5. 스프링 어노테이션
                @ 어노테이션 활용 : 개발자는 단순한 어노테이션을 전달함으로써 내부적으로 복잡한 코드가 실행된다.
